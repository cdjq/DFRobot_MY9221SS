 ...
 * @brief 通过宏定义设置多个指定引脚的亮度，请将12颗单色LED灯的负极连在12个ABC引脚上，正极接上电源
...
//   }
  ...
  //流水灯
  for (int i = C0; i <= A3; i<<=1) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }
  //流水灯，反向
  for (int i = A3; i >= C0; i>>=1) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }

  //级别灯
  for (int i = C0; i <= 4095; i = i<<1 + C0) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }
  
  //反向级别灯
  for (int i = A3; i >= 4095; i = i>>1 +A3) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }

  //弹跳灯，两个灯来回跳跃
  for (int i = B0|C0; i <= A3|B3; i<<=2) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }
  for (int i = A3|B3; i >= B0|C0; i>>=2) {
      rgbDriver.setMonochromeLed(/*pinNo=*/i, /*brightness=*/255);
      delay(1000);
  }
}