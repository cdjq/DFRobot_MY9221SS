 ...
 * @n 驱动每次收到新数据会将旧数据往后传递，例如有两个驱动级联，发送两次数据，第二次的数据控制的是第一个驱动，第一次的数据控制的是第二个驱动，完成后再发送锁存信号，两个驱动工作。如果发送一个数据就锁存，数据控制的是第一个驱动，而第二个驱动的状态将继承第一个驱动的状态
...
void loop() {
  ...
  */
  ledDriver.setMonochromeLed(/*pinNo=*/A3|B3|C3, /*brightness=*/0xff);//较远驱动上的A3B3C3引脚上的灯
  ledDriver.setMonochromeLed(/*pinNo=*/A3|B3|C3, /*brightness=*/0xf); //较近驱动上的A3B3C3引脚上的灯
  ...
  */
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf, /*G=*/0,/*B=*/0);//第二个驱动上的灯
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xff,/*G=*/0,/*B=*/0);//第一个驱动上的灯
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0xff,/*B=*/0);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0xf, /*B=*/0);
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0,/*B=*/0xff);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0,/*B=*/0xf );
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0xff,/*B=*/0xff);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0,/*G=*/0xf, /*B=*/0xf );
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xff,/*G=*/0,/*B=*/0xff);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf, /*G=*/0,/*B=*/0xf );
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xff,/*G=*/0xff,/*B=*/0);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf, /*G=*/0xf, /*B=*/0);
  ledDriver.latch();//发送锁存信号使所有驱动工作
  delay(1000);


  //此处模式不同，使灯的亮度不一样
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf,/*G=*/0xf,/*B=*/0xf);
  /**
  * @brief 设置灰阶为16位，其值为3
  * @param temp 保留位元
  ...
  */
  ...
  //模式相同，灯的亮度一样
  //恢复灰阶模式为8位
  ledDriver.setMode(/*temp=*/0, /*hspd=*/ledDriver.eIoutSlow, /*bs=*/ledDriver.eBit8,\
                    /*gck=*/ledDriver.eOriginalFreq, /*sep=*/ledDriver.eAPDM, /*osc=*/ledDriver.eInternalGCK,\
                    /*pol=*/ledDriver.eLedDriver, /*cntset=*/ledDriver.eFreeRunning, /*onest=*/ledDriver.eRepeat);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf,/*G=*/0xf,/*B=*/0xf);
  ledDriver.setRgbLed(/*ledNo=*/LED3,/*R=*/0xf,/*G=*/0xf,/*B=*/0xf);
